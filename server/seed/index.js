(async () => {
	const seed_business_units = require('./seed_business_units')
	await seed_business_units.loadBusinessUnits()
	const seed_roles = require('./seed_roles')
	await seed_roles.loadRoles()
	const seed_access_codes = require('./seed_access_codes')
	await seed_access_codes.loadAccessCodes()
	const seed_users = require('./seed_users')
	await seed_users.loadUsers()
	const seed_business_units_users = require('./seed_business_units_users')
	await seed_business_units_users.loadBusinessUnitsUsers()
	const seed_abbreviations = require('./seed_abbreviations')
	await seed_abbreviations.loadAbbreviations()
	const seed_countries = require('./seed_countries')
	await seed_countries.loadCountries()
})().catch(e => console.error(e.stack))
